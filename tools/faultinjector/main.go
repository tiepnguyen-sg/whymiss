...
func RunScenario(ctx context.Context, s Scenario, enclave, beaconAPI, outDir string) error {
	...
	observations, err := buildObservations(s, dutySlot, slotStart, dutyAt, blockFound, blockRoot, proposerIndex, seenAt, included, includedInSlot, includedAt)
	if err != nil {
		return err
	}
	...
	readme := renderReadme(s, dutySlot, blockFound, included)
...
func findCleanDuty(ctx context.Context, obs *Observer, startEpoch, validatorIndex uint64, avoid *[2]uint64) (slot uint64, d duty, at time.Time, err error) {
	const lookahead = 3 // bounded: an epoch search that never terminates is a bug, not patience
	for epoch := startEpoch; epoch < startEpoch+lookahead; epoch++ {
		slot, d, at, err := obs.FetchDuty(ctx, epoch, validatorIndex)
		if err != nil {
			return 0, duty{}, time.Time{}, fmt.Errorf("epoch %d: %w", epoch, err)
		}
		if avoid == nil {
		...
		fmt.Printf("faultinjector: epoch %d's duty slot %d has proposer %d, inside the fault's own range %v — trying the next epoch\n",
			epoch, slot, proposer, *avoid)
	}
	return 0, duty{}, time.Time{}, fmt.Errorf("no clean duty found in %d epochs from %d", lookahead, startEpoch)
}
...
func renderReadme(s Scenario, slot uint64, blockFound, included bool) string {
...
`, s.ID, s.Description, s.Fault.Kind, s.Target, s.Duration, slot, blockFound, included,
