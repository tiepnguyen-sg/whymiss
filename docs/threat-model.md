...
A separate, unprivileged process that periodically calls configured beacon/metrics
endpoints, samples a configured NTP server, optionally reads a few files under
`/proc`, writes what it observed to a local SQLite file, and — when a duty is missed
or late — runs a pure, offline rule engine over those observations to name a cause.
...
│  CL metrics  <──(read-only HTTP, optional)─────────────┤ │
│  baseline node/metrics <──(read-only, optional)────────┤ │
│  NTP server  <──(SNTP/UDP, operator-configured)────────┤ │
...
| **Data exfiltration / unwanted network egress.** whymiss phones home, checks for updates, or is tricked into talking to an attacker-controlled host. | Zero outbound calls except to endpoints the operator explicitly configured (I-4): `--beacon-api`, optional `--cl-metrics-api`, optional `--baseline-beacon-api`/`--baseline-metrics-api`, and repeatable `--ntp-server`. No telemetry, version checks, analytics, or implicit defaults. `make check.egress` guards HTTP construction outside the adapter boundary; NTP is confined to `internal/clock`. Host-level observations (`/proc/pressure/*`, `/proc/stat`) never leave the box. |
...
| **Supply-chain compromise via a dependency.** A malicious or compromised third-party module ships inside the binary. | Dependency austerity (I-14): every new module needs an ADR. `govulncheck` runs in every `make ci`; release artifacts include per-archive SBOMs, a keyless-cosign-signed checksum, and SLSA provenance generated by the isolated release job described in ADR-0010. |
...
- **A malicious or spoofed metrics/NTP source**, if the operator points an endpoint
  flag at something they do not trust.
...
The fresh-install workflow runtime-checks the container's non-root identity,
read-only root filesystem, dropped capabilities, and absence of a shell. After a
systemd install, audit the effective unit on that host with
`systemd-analyze security whymiss.service`; distro and systemd versions affect the
score, so the shipped directives are not represented as a universal numeric grade.
