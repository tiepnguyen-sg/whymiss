...
- [ ] Switching between pre-ePBS and post-ePBS timing requires **only a config change** — proven by test — **the config-driven half is now proven; the post-ePBS half cannot be, because those fields do not exist yet.** `TestAnalyze_TimingFollowsTheScheduleNotTheCode` (added 2026-08-27) replays the real `p2p-degraded-prysm-r06` record twice against two schedules and asserts the verdict differs: at mainnet's 4s attestation deadline it is `local.p2p_degraded`, and at an 8s deadline the same observations become `unknown.no_rule_matched`. No code changes between the two runs, so the rules demonstrably read their deadlines off `SlotSchedule` rather than from literals. Nothing tested this before: every other test in the repository analyses with mainnet timing, so a rule hard-coding `4 * time.Second` would have passed all of them. What remains is task 5.5's payload-reveal and PTC deadlines — `SlotSchedule` has no fields for them yet, and its doc comment says adding them is additive. Close this once those fields exist and a post-ePBS schedule can actually be constructed.
