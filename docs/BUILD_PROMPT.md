...
- [x] 72-hour soak against Hoodi testnet: RSS stays under the documented ceiling, disk respects the byte cap, zero goroutine leaks (`goleak`) — **passed 2026-08-30.** `goleak` is not a soak assertion at all: `internal/app/main_test.go` calls `goleak.VerifyTestMain(m)`, so every test in the package runs under it, including `TestWatch_EveryCollectorShutsDownCleanly`, which enables every optional collector at once and asserts the whole daemon unwinds on cancellation. That passes in `make ci` today. The duration half ran from 2026-08-27T01:44:21Z to 2026-08-30T01:45:18Z on binary `1d0bdd6d5660219b73f7bb10196222f6e67cfb0b5c7750d8bd60853f6421d08d`, and `test/soak/run.sh` decided pass/fail itself — it exits non-zero the moment RSS exceeds 262144 KiB or the database exceeds 104857600 bytes, and writes `result=PASS` to `summary.txt` only on success. It wrote `result=PASS`, over 4321 one-minute samples, with **`max_rss_kib=34688`** (13.2% of the ceiling) and **`max_database_bytes=25447600`** (24.3% of the cap); 674 verdicts were recorded in that window. The evidence is committed at `test/soak/evidence/20260827T014421Z/` — the summary, the samples, the full log, and the binary sha, with the commands to recompute every figure above from the raw data.
  - **The watcher's own error count was wrong, and the real number is 55, not zero.** `~/phase2_watch.sh` printed `errors : 0` because it greps `level=ERROR` (logfmt) while the daemon emits JSON (`"level":"ERROR"`) — a defect in the helper script, since fixed, not in whymiss, but an item closed on a broken measurement is worse than an item left open. All 55 lines were read and attributed: **49** are `cannot confirm slot <n> as seen or skipped: node is not fully synced, execution-valid, and past the slot after waiting 1m30s` (the node behind the public gateway lagging), **5** are `fetch block header head: ... unexpected status 500: dialing ... timed out` (the gateway itself), and **1** is the `context canceled` raised when the soak stopped the daemon at the end. None originates inside whymiss, and none is a resource or leak signal — which is what this item measures.
  - **The soaked binary is machine-code-identical to the released tree.** Its sha does not match a fresh build, because it was built at `0c0a94b` with a dirty tree while HEAD has since moved. Rebuilding HEAD for `linux/amd64` with the soaked binary's own version string gives the same size (17019042 bytes) and differs in exactly **201 bytes across 5 regions, all of them build metadata**: the Go and GNU build IDs (120 B), the module pseudo-version (2 × 20 B), and `vcs.revision` plus `vcs.time` (2 × 75 B). No instruction or string data differs. This is what turns "a soak passed" into "the artifact being released passed", and it is why no file under `cmd/` or `internal/` was touched while closing this item.
...
- [x] Fresh Linux box → **running** in under 5 minutes following the README alone — `.github/workflows/fresh-install.yml` runs two jobs on `ubuntu-latest`, a genuinely fresh Linux box, both inside a 10-minute timeout: `from-source` follows the README's "From source" path (`make build`) and then fails loudly if any flag the README's example names has been renamed, and `docker-compose` runs `make test.freshinstall`, which brings the stack up and asserts Prometheus is scraping the daemon. Both passed 2026-08-27.
- [x] Fresh Linux box → **first verdict within two epochs** — measured on the Hoodi soak, a real Linux host against a real beacon node: the daemon started at 01:44:21Z and recorded its first verdict at 01:56:00Z, **11m39s**, with the second at 02:02:24Z — 6m24s later, which is one epoch. Two epochs (12.8 min) is the worst case: a duty is discovered for the following epoch, then the duty slot has to arrive, then its collection window has to close.

  > **These two items were one, and it could never be met as written.** The original read "running *and producing verdicts* in under 5 minutes", which folds two different clocks into one bar. Install speed is under whymiss's control; the first verdict is not — an attester duty arrives once per epoch (32 slots x 12s = 6.4 min) and no verdict can exist before that duty's collection window closes. The 11m39s and the one-epoch cadence between verdicts are the evidence. Split rather than dropped: both halves of the original intent — an operator sees it running fast, and sees it working soon after — are still required, each now measured against the clock that actually governs it.
...
- [x] Release artifacts carry provenance and signatures; verification steps documented and tested — **closed 2026-08-30 by publishing `v0.2.1`**, the first release this pipeline has ever carried end to end (0.1.0's tag never produced a release, so `verify published artifacts`, the GHCR publish, and the un-drafting had never once executed). "Tested" here means the README's own commands were run from outside the workflow, against the published assets, by someone holding nothing but the tag name: `sha256sum -c` matched all four artifacts; `cosign verify-blob` against `checksums.txt.bundle` with the keyless identity `.../release.yml@refs/tags/v0.2.1` returned `Verified OK`; `slsa-verifier verify-artifact` returned `PASSED: SLSA verification passed`, naming builder `generator_generic_slsa3.yml@refs/tags/v2.1.0` at commit `c1b8819`; and `cosign verify ghcr.io/tiepnguyen-sg/whymiss:v0.2.1` validated the image signature at digest `sha256:9d81aad1…`. The image is anonymously pullable — an unauthenticated GHCR token fetches the manifest with HTTP 200 — so the Compose quickstart works for a reader who has never authenticated.
  - The route there is worth recording, because it cost a version number. `v0.2.0` built, signed, and generated provenance, then failed at `verify published artifacts`: that job ran with `contents: read`, and GitHub reveals draft releases only to tokens with push access, so `gh release download` answered `release not found`. Release tags are immutable by ruleset and a `push`-triggered workflow re-runs the workflow file from the tag's own commit, so the fix could not rescue the tag it fixed. That is the trade-off the ruleset was chosen for, not a defect in it.
...
- [x] Network baseline is opt-in and defaults to off; tool is fully functional with it disabled — off by default because `internal/config`'s `Defaults()` sets no value for `watch.baseline_beacon_api`, so it is the empty string unless an operator supplies one, and `internal/app/watch.go` starts the baseline collector only inside `if cfg.BaselineBeaconAPI != ""`. Functional without it is not reasoning here but a running measurement: the release soak carries **no baseline flag at all** and had produced 50 verdicts with zero ERROR lines over 5h31m when checked on 2026-08-27 — duties tracked, healthy and degraded outcomes both recorded, metrics exported. The documented consequence is that R-110 and R-200 cannot fire, which is measured rather than asserted: stripping the baseline from every corpus record that carries one changes 15 verdicts, all to `unknown.insufficient_data` (see the accuracy-improvement item above). Functional, with two of fourteen causes out of reach — which is what "opt-in" costs and why `doctor` now warns when it is unset.
- [x] Measured accuracy improvement documented with before/after numbers — measured 2026-08-27 by replaying every corpus record twice, once intact and once with its `network_baseline_sampled` observation removed, and diffing `rca.Analyze`'s cause. Sixteen records carry a baseline; removing it changes the verdict on **fifteen**, and every one collapses to `unknown.insufficient_data` — `network.late_block` x5 and `local.p2p_degraded` x10. Corpus-wide that is 41 named causes and 11 refusals (21.2%) with the baseline, against 26 and 26 (50%) without, and causes exercised falling from 8 to 6. The two that vanish are exactly the two that answer "was it me or the network", so the baseline is not an accuracy tweak — it is what makes the product's central question answerable at all. **This measures the single-node baseline delivered early by ADR-0025, not a Xatu percentile baseline**, which does not exist yet (task 5.1); adding one needs its own before/after against these numbers.
- [x] Switching between pre-ePBS and post-ePBS timing requires **only a config change** — proven by test — **closed 2026-08-30; both halves now hold.** The config-driven half was already proven by `TestAnalyze_TimingFollowsTheScheduleNotTheCode`, which replays the real `p2p-degraded-prysm-r06` record twice against two schedules and gets `local.p2p_degraded` at a 4s attestation deadline and `unknown.no_rule_matched` at 8s, with no code change between the runs. The post-ePBS half was blocked on fields that did not exist; `domain.SlotSchedule` now carries `PayloadRevealDeadline` and `PTCDeadline`, additively, exactly as its doc comment said they would arrive. `TestLoadSwitchesToPostEPBSTimingByConfigurationAlone` loads the same binary twice and gets a pre-ePBS schedule from no configuration and a post-ePBS one — deadlines resolving against the slot start — from a YAML file, with nothing differing between the runs but that file. Both deadlines also arrive through `WHYMISS_PAYLOAD_REVEAL_DEADLINE` and `WHYMISS_PTC_DEADLINE` like every other setting.
  - **No post-ePBS default ships anywhere in the codebase, and that is the design.** The spec values are not final, and a plausible-looking constant compiled in would be indistinguishable from a measured one at the moment it produced a wrong verdict (I-8). `IsPostEPBS()` is decided by whether a payload-reveal deadline is configured, never by a fork name — whymiss never asks which fork is running, only what the timing model says, which is the whole point of the item. `Validate` rejects a half-configured pair at load: a `ptc_deadline` with no `payload_reveal_deadline` names no boundary for the committee to vote against, and the accessors return `(zero, false)` rather than the slot start, so a caller that ignores the bool cannot read "no deadline exists" as "the deadline already passed".
  - What this does **not** claim: no rule consumes either deadline yet. That is task 5.5, and the DoD item below it — validation against a Glamsterdam devnet — is where those rules earn their evidence.
- [ ] ePBS rules validated against a Glamsterdam devnet — **still open, and the reason changed on 2026-08-30.** An earlier note here said no ePBS devnet existed; that was wrong. `ethereum-package` builds one from the same tooling this repo already uses (`fulu_fork_epoch`, `gloas_fork_epoch`, one supernode), and a three-node Glamsterdam devnet ran that day with Gloas active at epoch 2, fork version `0x80000038`.
  - Running against it found a defect that made the product useless there, and it was not in a rule. whymiss asserted the Electra invariant `attestation.data.index == 0`; EIP-7732 repurposes that field to signal payload availability, so `CheckInclusion` rejected every block, no `attestation_included` was ever written, and every slot resolved to `no observations recorded`. Measured across the devnet's own fork boundary: **23 of 32 attestations in 13 post-fork blocks carried a non-zero index, against 0 of 32 in the 32 blocks before it.** Fixed, with the recorded block as a fixture, and re-verified live: zero errors where the same setup had produced ten in minutes.
  - Also landed from that session: the slot schedule is now read from the node's own `/eth/v1/config/spec` (ADR-0026), which on that chain yields a 3s attestation deadline, a 6s payload-reveal deadline and a 9s PTC deadline with no configuration at all.
  - **Correction, same day: a healthy public Glamsterdam network exists and whymiss runs on it.** An earlier version of this item said no healthy ePBS network was available and the work had to wait for Sepolia on 2026-09-28. That was wrong because nobody looked. `beacon.glamsterdam-devnet-8.ethpandaops.io` and `beacon.plataberget.ethpandaops.io` serve a live Gloas chain (fork `0x80733183`, active from epoch 1536) which measured **41 blocks in 41 slots with its head exactly at wall clock** on 2026-08-30. whymiss connected to it, logged `slot schedule adopted from the node's own spec ... payload_reveal_deadline=6s ptc_deadline=9s post_epbs=true`, and collected duties end to end without a single error — `attestation_included` and `collection_completed` recorded, four verdicts written — against **Nimbus**, a client this project has never written an adapter for, which is independent evidence for both ADR-0026's design and the same day's post-ePBS collection fix.
  - **What is actually blocked is fault injection, not access.** A corpus record for `network.payload_late` needs a payload made late deliberately, and nobody may do that to a shared testnet. That needs a local devnet, and the local devnet is what fails: Three configurations were tried on 2026-08-30 and are tabulated in [ADR-0027](adr/0027-network-payload-late.md): with an ePBS builder the chain still loses about half its slots, and starting genesis directly at Gloas makes Lighthouse refuse to boot on an invalid genesis state. `network.payload_late` is approved in principle by that ADR and deliberately left out of `domain.CauseIDs()` and `docs/causes.md` until a rule and a first corpus record can land with it, so the public taxonomy never advertises a cause nothing emits.
  - **Task 5.5 landed 2026-08-30, and this item is closer but not closed.** `network.payload_late` (ADR-0027), the `payload_attested` observation kind and rule R-120 are implemented and were validated against the live public Glamsterdam network — the daemon adopted the post-ePBS schedule from the node's own spec and recorded `payload_attested {payload_present, ptc_votes}` from real blocks. Running it there also caught a defect unit tests missed: R-120 first fired on the committee's vote alone, which on a chain where 32 of 51 votes report the payload absent would have handed a cause to every healthy duty; it is now gated on `dutyHasObservableLoss`.
  - **What keeps the item open is the corpus.** The cause has no scenario, so it is unmeasured, and the manifest format admits only injected faults (`fault_kind`, `fault_target`, a duration) while this condition occurs naturally on a public network and must not be injected into a shared testnet. Whether the corpus should admit observed records is a decision about what the corpus means and is deliberately left open.
  - The older note below stands for the record. No rule reads `PayloadRevealDeadline` or `PTCDeadline` directly for *timing* attribution, so there is nothing yet whose verdicts a devnet could validate — that is task 5.5. And the devnet used here was configured with `mev_type: null`, so it has no ePBS builder: it produced blocks on 10 of 41 slots and whymiss correctly reported the rest as skipped. Closing this item needs both a rule to test and a devnet healthy enough to test it on.
- [x] Case study reproduces a known public root cause end to end — [`docs/case-studies/2023-05-mainnet-finality.md`](case-studies/2023-05-mainnet-finality.md), written 2026-08-30. The May 2023 mainnet finality incidents lost finality twice in 24 hours with participation down to ~75% at epoch 411,448, and their published root cause is a chain whose **first link** is an execution client that stops responding — steps 3-5 are a consensus-client cache bug, out of scope for a node-local tool, and the study says so rather than claiming the whole incident.
  - That first link is reproduced as a real fault on a real devnet, not simulated: `test/corpus/el-slow-cpu` caps the watched attester's own execution client to 8% of one core under sustained transaction load, and whymiss reports `local.el_slow` with Engine calls totalling **9970.78ms against that node's own 491.23ms rolling p99** — 20.3x its normal behaviour — while `block_seen` at +0.50s shows propagation was fine. The study quotes the verdict unedited, including why confidence is `medium` rather than `high`.
  - Writing it caught a documentation defect worth naming: `CHANGELOG.md` quoted this record as "16,295ms against a 981.8ms baseline" with `head_updated` at +25.19s. Those figures belong to a bisection run that was never admitted; the committed record replays to the numbers above. Both occurrences are corrected, and the corrections say where the old ones came from.
