...
func TestVCSlow(t *testing.T) {
	t.Run("matches when signing dominates across a timely head and late publish", func(t *testing.T) {
		tl := timelineWith(t,
	...
	t.Run("does not match when head_updated is missing", func(t *testing.T) {
		tl := timelineWith(t,
			mustObs(t, domain.ObsBlockSeen, offset(1670*time.Millisecond), nil),
			mustObs(t, domain.ObsAttestationPublished, offset(4180*time.Millisecond), nil),
		)
	...
	t.Run("does not match when block_seen is itself after the deadline", func(t *testing.T) {
		tl := timelineWith(t,
			...
			mustObs(t, domain.ObsHeadUpdated, offset(5100*time.Millisecond), nil),
	...
	t.Run("does not match when validation dominates", func(t *testing.T) {
		tl := timelineWith(t,
			mustObs(t, domain.ObsBlockSeen, offset(500*time.Millisecond), nil),
			mustObs(t, domain.ObsHeadUpdated, offset(3800*time.Millisecond), nil),
			mustObs(t, domain.ObsAttestationPublished, offset(4200*time.Millisecond), nil),
		)
		if _, ok := (VCSlow{}).Evaluate(tl, defaultCfg); ok {
			t.Fatal("want no match when signing is not dominant")
		}
	})
}
