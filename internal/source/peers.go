...
func (s *MetricsSampler) SamplePeerCount(ctx context.Context, client ConsensusClient, metricsURL string) (domain.MetricSample, error) {
	switch client {
	case ConsensusLighthouse:
		return s.scraper.SampleLighthousePeerCount(ctx, metricsURL)
	case ConsensusPrysm:
		return s.scraper.SamplePrysmPeerCount(ctx, metricsURL)
	default:
